// ============================================================
// POST /api/summarize-monthly — Generate monthly summary
// ============================================================

import { NextRequest } from 'next/server';
import { deepseekChat } from '@/lib/deepseek';
import { MONTHLY_SUMMARY_PROMPT } from '@/lib/prompts';
...
export async function POST(request: NextRequest) {
  try {
    const { year, month, records } = await request.json();
    ...
    if (!Array.isArray(records) || records.length === 0) {
      return Response.json(
        { error: 'No records for this month' },
        { status: 404 }
      );
    }
    ...
    const messages = [
      { role: 'system' as const, content: MONTHLY_SUMMARY_PROMPT },
      {
        role: 'user' as const,
        content: `这是 ${year} 年 ${month} 月的记录：\n\n${JSON.stringify(summaries, null, 2)}\n\n请为这个月生成总结。只输出JSON，不要其他内容。`,
      },
    ];
